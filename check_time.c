...
int main(void)
{
  const char *hexInput = "4e6f772069732074";
  ...
  size_t dataSize = 64 * 1024 * 1024;
  unsigned char *largeData = (unsigned char *)malloc(dataSize);
  if (!largeData)
  {
    free(inputBytes);
    fprintf(stderr, "Error: malloc failed for largeData\n");
    return 1;
  }

  for (size_t i = 0; i < dataSize; i++)
  {
    largeData[i] = inputBytes[i % inputLen];
  }

  unsigned char *ciphertext = (unsigned char *)malloc(dataSize);
  if (!ciphertext)
  {
    free(inputBytes);
    free(largeData);
  ...
  {
    uint64_t block = 0;
    memcpy(&block, largeData + i, 8);
    block ^= iv;
    block = des_encrypt_block_with_print(block, subkeys);
  ...
  printf("Encryption of 64 MB took %.2f seconds\n", time_spent);
